...
static esp_err_t esp_modem_dce_common_handle_imei(esp_modem_dce_t *dce, const char *line)
{  
    esp_err_t err = ESP_FAIL;
    if (strstr(line, MODEM_RESULT_CODE_ERROR)) {
        err = esp_modem_process_command_done(dce, ESP_MODEM_STATE_FAIL);
    ...
    if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
        char *temp_data = dce->handle_line_ctx;
        sscanf(line,"%s", temp_data);
        err = ESP_OK;
    }
...
static esp_err_t esp_modem_dce_common_handle_iccid(esp_modem_dce_t *dce, const char *line)
{  
    esp_err_t err = ESP_FAIL;
    if (strstr(line, MODEM_RESULT_CODE_ERROR)) {
        err = esp_modem_process_command_done(dce, ESP_MODEM_STATE_FAIL);
    ...
    if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
        char *temp_data = dce->handle_line_ctx;
        //+ICCID: 89861120040212010789
        sscanf(line,"%*s %s", temp_data);
        err = ESP_OK;
    }
...
{
    return esp_modem_dce_generic_command(dce, "AT+CGSN\r", MODEM_COMMAND_TIMEOUT_DEFAULT,esp_modem_dce_common_handle_imei, result);
}
esp_err_t esp_modem_dce_get_iccid_number(esp_modem_dce_t *dce, void *param, void *result)
{
    return esp_modem_dce_generic_command(dce, "AT+CICCID\r", MODEM_COMMAND_TIMEOUT_DEFAULT,esp_modem_dce_common_handle_iccid, result);
}
esp_err_t esp_modem_dce_set_cgauth(esp_modem_dce_t *dce, void *param, void *result)
{
    esp_err_t err = esp_modem_dce_generic_command(dce,  (char *)param, MODEM_COMMAND_TIMEOUT_DEFAULT, esp_modem_dce_handle_response_default, NULL);
    return err;
}
