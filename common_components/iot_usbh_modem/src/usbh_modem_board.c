...
extern void modem_board_call_back(uint8_t type,int count,...);
                           ...
                           int32_t event_id, void *event_data)
{
    if (event_base == IP_EVENT) {
        ...
        if (event_id == IP_EVENT_PPP_GOT_IP) {
            ...
            modem_board_call_back(3,5,(uint8_t *)&event->ip_info.ip,(uint8_t *)&event->ip_info.netmask, (uint8_t *)&event->ip_info.gw,(uint8_t *)&s_dns_ip_main.u_addr.ip4,(uint8_t *)&s_dns_ip_backup.u_addr.ip4);
            if (s_modem_evt_hdl) {
            ...
            modem_board_call_back(4,1,(uint8_t *)&event->ip6_info.ip);
        }
...
static void _modem_daemon_task(void *param)
{
    ...
    modem_board_call_back(5,1,&dce_config.pdp_context.apn);
    ...
    while (true) {
        ...
        case STAGE_START_PPP:
        	modem_board_call_back(1,4,esp_modem_dce_get_imei_number,esp_modem_dce_get_iccid_number,esp_modem_dce_set_cgauth,s_dce);
