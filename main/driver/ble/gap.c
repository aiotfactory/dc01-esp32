...
static uint8_t esp_uri[] = {BLE_GAP_URI_PREFIX_HTTPS, '/', '/', 'z', 'h', 'i', 'y', 'i', 'n', 'c', 'e', '.', 'c', 'o', 'm'};
...
static void set_random_addr(void) {
	...
	addr.val[0]=rt_device_mac[5];
	addr.val[1]=rt_device_mac[4];
...
static int gap_event_handler(struct ble_gap_event *event, void *arg) {
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            ...
            pkey.passkey = device_config_get_number(CONFIG_BLE_PASSWORD);
