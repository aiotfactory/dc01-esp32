...
/**
 * 最近成功通讯的时间。
 * The time of the last successful communication.
 * 使用这个变量可以帮助监控设备的连接状态，判断是否有异常长时间没有进行通信。
 * This variable can help monitor the connection status of the device and determine if there has been an 
 * unusually long period without communication.
 */
extern uint32_t last_comm_seconds;
 ...
 * 
 * callback 用户配置的回调函数，用于在初始化过程中执行用户自定义配置。
 * module_max 模块的最大值，模块ID要顺序定义。
 * config_version 指定本次配置的版本号，用于区分不同的配置版本。
 * server_host 上报数据的云端主机地址，通常是一个域名或IP地址。
 * server_port 上报数据的云端主机端口号，用于指定通信的服务端口。
 * core_log 是否开启内核日志功能，开启后可以记录内核运行的关键信息，便于调试。
 * mem_debug 是否开启内存诊断日志功能，一般仅在怀疑存在内存泄漏时启用，会增加额外开销。
 * cloud_log 是否开启云端日志功能，注意此功能会消耗较多流量，特别是使用4G模块上报时需谨慎。
 * 
 ...
 * 
 * callback: A user-defined callback function executed during initialization for custom config.
 * module_max specifies the maximum value of the module. Module IDs should be defined in sequential order.
 * config_version: Specifies the version of the current configuration to distinguish between different versions.
 * server_host: The cloud host address (usually a domain name or IP) for data reporting.
 * server_port: The cloud host port number for communication.
 * core_log: Enables or disables kernel logging, which records key runtime information for debugging.
 * mem_debug: Enables or disables memory diagnostic logging, typically used only when suspecting memory leaks, 
 *            as it incurs additional overhead.
 * cloud_log: Enables or disables cloud logging, which can consume significant traffic, especially when using a 4G module.
 */
void system_init(config_callback callback, uint32_t config_version, uint32_t module_max,char *server_host, uint32_t server_port, uint8_t core_log, uint8_t mem_debug, uint8_t cloud_log);
