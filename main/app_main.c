...
void loop_wait_on_interrupt(uint32_t max_tickets)
{
	...
	if(util_get_run_seconds()>(last_comm_seconds+device_config_get_number(CONFIG_NO_COMMUNICATE_SECONDS)))
	{
		ESP_LOGE(TAG, "upload failed too long %lu>%lu+%lu",util_get_run_seconds(),last_comm_seconds,device_config_get_number(CONFIG_NO_COMMUNICATE_SECONDS));
		util_reboot(9);
	}
}
/**
 * 设备上电后首先调用的入口函数。
 * 该函数是整个应用程序的起点，设备在上电或复位后会自动调用此函数以初始化系统并启动主要功能。
 * 在该函数中，通常会完成系统的基础配置、硬件初始化、任务创建以及启动其他必要的服务或模块。
 * 开发者可以在该函数中实现自定义的初始化逻辑，例如设置回调函数、启动定时器或注册事件处理程序。
 * 
...
void app_main(void)
{
	uint8_t enable_mem_debug=0;
	uint8_t enable_core_log=0;
	uint8_t enable_cloud_log=0;
	char *host_server="120.26.167.61";
	uint32_t host_port=7000;  
    
  	system_init(define_user_config,1,MODULE_MAX,host_server,host_port,enable_core_log,enable_mem_debug,enable_cloud_log);//don't comment it out or move to other position
	...
	while(1)
	{
		...
		for(uint8_t i=0;(i<task_idx)&&(all_tasks[i].status>0);i++)
		{
			...
			if((all_tasks[i].status)&&((all_tasks[i].exe_times==0)||((util_get_run_seconds()-all_tasks[i].exe_last_seconds)>device_config_get_number(all_tasks[i].task_upload_interval_idex))))
			{
