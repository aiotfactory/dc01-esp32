...
static void uart_init(uint8_t tx_pin_in,uint8_t rx_pin_in,uint32_t baud_rate_in)
{
	...
	{
		ESP_LOGI(TAG, "uart_init tx %u rx %u rate %lu", tx_pin_in,rx_pin_in,baud_rate_in);
		if((init==1)&&((tx_pin!=tx_pin_in)||(rx_pin!=rx_pin_in)||(baud_rate!=baud_rate_in)))
		{
			device_config_set_number(CONFIG_UART_TX_PIN,tx_pin_in);
			device_config_set_number(CONFIG_UART_RX_PIN,rx_pin_in);
			device_config_set_number(CONFIG_UART_BAUD_RATE,baud_rate_in);
			device_config_set_number(CONFIG_UART_ONOFF,1);						
			config_write();
			tcp_send_config("uart_config_changed",NULL,0);
		}
		if(driver_install==0)
		{
			ESP_ERROR_CHECK(uart_driver_install(UART_NUM_1, 256, 256, 10, &uart0_queue, 0));	
			driver_install=1;
		}
	    ...
	    ESP_ERROR_CHECK(uart_set_pin(UART_NUM_1, tx_pin_in, rx_pin_in, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
		init=1;		
		tx_pin=tx_pin_in;
		rx_pin=rx_pin_in;
		baud_rate=baud_rate_in;
...
{
	uart_init(device_config_get_number(CONFIG_UART_TX_PIN),device_config_get_number(CONFIG_UART_RX_PIN),device_config_get_number(CONFIG_UART_BAUD_RATE));
	if(init)
	{
		uint8_t *temp_buf;
...
void uart_util_queue(void)
{
	if(init==0)
		return;
    uart_event_t event;
    if (xQueueReceive(uart0_queue, (void *)&event, 0)) {
        switch (event.type) {
        case UART_DATA:
            //ESP_LOGE(TAG, "uart_util_queue %d", event.size);
            rx_times++;
            uint8_t *rx_buf=user_malloc(event.size+2);
            int rx_len=uart_read_bytes(UART_NUM_1, rx_buf+1, event.size, 0);
            if(rx_len<=0){
				user_free(rx_buf);
            	rx_times_failed++;
            }else
        ...
        case UART_BREAK:
        	rx_break_times++;
            ESP_LOGW(TAG, "uart1 rx break");
            break;
        //Event of UART parity check error
        case UART_PARITY_ERR:
        	rx_parity_err_times++;
            ESP_LOGW(TAG, "uart1 parity error");
            break;
        //Event of UART frame error
        case UART_FRAME_ERR:
        	rx_frame_err_times++;
            ESP_LOGW(TAG, "uart1 frame error");
            break;
        //Others
        default:
...
void uart_util_tx(uint8_t *command_data,uint32_t command_data_len)
{	
	uint32_t idex=0;
	...
	if(baud_rate>0)
		uart_init(tx_pin,rx_pin,baud_rate);
	...
	if((init==0)||(tx_data_len==0))
		return;
	tx_times++;
	ESP_LOGI(TAG, "uart_util_tx len %lu", tx_data_len);
	cloud_printf_format(NULL, NULL, "%02x",tx_data,tx_data_len);
	int tx_len=uart_write_bytes(UART_NUM_1, tx_data, tx_data_len);
	if(tx_len<=0)
		tx_times_failed++;
	else
		tx_length+=tx_len;
